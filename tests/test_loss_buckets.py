...
from model_based_curation import split_by_loss
...
def test_split_by_loss_writes_yaml_lists_named_by_bucket_interval():
    dataset_dir = _temp_dir("mapped_dataset_streaming")
    output_dir = _temp_dir("bucket_output_streaming")
    ...
    output_paths = split_by_loss(
        dataset_dir,
        [0.5, 1.5],
        output_dir,
        loss_fn=lambda ex: {1: 0.2, 2: 0.9, 3: 2.3}[int(ex["id"])],
    )
